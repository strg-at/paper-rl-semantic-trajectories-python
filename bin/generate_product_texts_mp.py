...
"""
    ...
    def process_batch(self, batch: list[dict]) -> list[tuple]:
        results = []
        for row in batch:
            try:
...
def worker_init(batch):
    worker = DescriptionWorker()
    return worker.process_batch(batch)


def generate_and_store_product_descriptions(dataset_parquet_file: str, batch_size: int, num_workers: int):
    ...
    # Tad more complicated query to make sure we don't reprocess the already processed products.
    # This allows to call the script multiple times to resume generation
    data = conn.sql(
        f"""
        WITH source_data AS (
    ...
    with mp.Pool(num_workers) as pool:
        with tqdm(total=total_rows, desc="Generating descriptions") as pbar:
            for batch_results in pool.imap_unordered(worker_init, batches):
                conn.executemany(
...
if __name__ == "__main__":
    ...
    generate_and_store_product_descriptions(dataset_parquet_file, batch_size, num_workers)
